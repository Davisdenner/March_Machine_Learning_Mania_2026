...
def _merge_features(df, team_features, seeds, elo_df, tourney_history, team_col, prefix,
    ...
    #team stats
    df = df.merge(
        team_features.add_prefix(f"{prefix}_").rename(columns={f"{prefix}_Season": "Season", f"{prefix}_TeamID": team_col}),
        on=["Season", team_col],
        how="left",
    )
    #seed
    seed_cols = seeds[["Season", "TeamID", "SeedNum"]].rename(columns={"TeamID": team_col, "SeedNum": f"{prefix}_Seed"})
    df = df.merge(seed_cols, on=["Season", team_col], how="left")

    #ELO
    elo_cols = elo_df[["Season", "TeamID", "ELO"]].rename(columns={"TeamID": team_col, "ELO": f"{prefix}_ELO"})
...
#========== calcular features diferenciais (TeamA - TeamB) =============#
...
#=============== construir o dataset de treino a partir dos resultados do torneio ================#
#cada jogo deve gerar um registro com TeamA (menor ID) vs TeamB (maior ID)
...
#=========== construir o dataset de submissão a partir do sample submission =============#
...
#============ retornar lista de colunas usáveis como features =================#
